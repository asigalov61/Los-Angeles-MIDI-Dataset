...
# Master MIDI Dataset GPU Search and Filter (ver. 4.0)
...
match_result_weight = 2 # @param {type:"slider", min:0.1, max:3, step:0.1}
match_length_weight = 1 # @param {type:"slider", min:0.1, max:3, step:0.1}
match_count_weight = 1 # @param {type:"slider", min:0.1, max:3, step:0.1}
          ...
          for target_sig in tqdm(src_signatures):
            ...
            if search_matching_type == 'Ratios':
              ...
              results = 1 - cp.divide((distances - cp.min(distances)), (cp.max(distances) - cp.min(distances)))
                ...
                signatures_data_normalized = cp.where(main_array_std != 0, (signatures_data - main_array_mean) / main_array_std, 0)
                target_sig_normalized = cp.where(target_array_std != 0, (target_sig - target_array_mean) / target_array_std, 0)
            ...
            results_weight = match_result_weight
            comp_lengths_weight = match_length_weight
            comp_counts_sums_weight = match_count_weight

            results = cp.divide(cp.add(cp.add(results_weight, comp_lengths_weight), comp_counts_sums_weight), cp.add(cp.add(cp.divide(results_weight, cp.where(results !=0, results, 1)), cp.divide(comp_lengths_weight, cp.where(comps_lengths_ratios !=0, comps_lengths_ratios, 1))), cp.divide(comp_counts_sums_weight, cp.where(comps_counts_sums_ratios !=0, comps_counts_sums_ratios, 1))))
